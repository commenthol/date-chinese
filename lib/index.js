...
var CalendarChinese = function () {
  ...
  _createClass(CalendarChinese, [{
    ...
    value: function _from(j, year) {
      ...
      var years = 1.5 + (ny - epoch) / base.BesselianYear;
    ...
    value: function midnight(jde) {
      var c = new julian.CalendarGregorian().fromJDE(jde);
      var ts = 0.5 - this.timeshiftUTC(c.toYear());
      var mn = Math.trunc(c.toJD() - ts) + ts;
